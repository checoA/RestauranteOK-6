app.controller('BarraParaLlevarController', [ '$scope', '$resource', 'barraParaLlevarService','messagesService','loginService','navegacion','$rootScope','cajaService', function($scope, $resource, barraParaLlevarService,messagesService,loginService,navegacion,$rootScope,cajaService) {
	...
	$scope.productosArray = barraParaLlevarService.obtener_productos();
	$scope.cantidad = '0';
	...
	function recuperarOrden(){
		barraParaLlevarService.recuperarOrden().then(function(datos){
		...
		$scope.productosVenta.push(p);
			$scope.total = Number(Number($scope.total) + Number(p.precio)).toFixed(2);
			 ayudante_total = Math.round($scope.total * 100) / 100
			    ...
			    $scope.total = Number($scope.total).toFixed(2);
		/*}*/
		barraParaLlevarService.enviarVentasParaLlevar($scope.productosVenta,$scope.numClientes).then(function(datos){
			if(datos.created){
				recuperarOrden();
	...
	$scope.agregarProducto = function(id){
		if($scope.cantidad!= '0'){
			barraParaLlevarService.obtener_x_id(id).then(function(datos){
				p = {};
	...
	$scope.eliminar_producto=function(elemento){
		...
		barraParaLlevarService.eliminarProducto(elemento).then(function(datos){
			//obtenerOrdenes();
	...
	$scope.enviarVenta=function(){
		if($scope.productosVenta.length > 0){
			barraParaLlevarService.enviarVentas($scope.productosVenta,$scope.numClientes).then(function(datos){
				if(datos.created){
					//recuperarOrden();
					$rootScope.chat.send("/app/setCocina", {}, JSON.stringify(datos));
					navegacion.setLocation("mesas");
	...
	$scope.cancelarVenta=function(){		
		barraParaLlevarService.cancelarVenta($scope.productosVenta);
		$scope.productosVenta = []
		$scope.total = 0;
	}
	...
	$scope.imprimirPreviaVenta = function(){
		...
		barraParaLlevarService.obtenerOrden($scope.orden[0].orden.id).then(function(orden){
			$scope.orden = orden;
	...
	function ya_existe_en_orden(orden){
		respuesta = false;
		for (var i = 0; i < $scope.productosVenta.length; i++) {
			if(orden.producto.id == $scope.productosVenta[''+i].producto.id && $scope.productosVenta[''+i].estado.id != 6 && $scope.productosVenta[""+i].estado.id !=5 && $scope.productosVenta[""+i].estado.id !=4){
	...
	$scope.venta=function(){
		$scope.numero_mesa = barraParaLlevarService.getMesa();
	...
	$scope.liberar = function(){
		$scope.orden = angular.copy($scope.productosVenta);
		barraParaLlevarService.liberarParaLlevar($scope.orden[0].orden);
		navegacion.setLocation("paraLlevar");
	}
